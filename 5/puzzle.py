...
def computer(intcode, system_ID=None):
    memory = [int(x) for x in intcode.split(',')]
    log = []
    out = {'memory':memory, 'log':log}
    address = 0
    while address != len(intcode):
        ...
        if DE == 99:
            ...
            memory[memory[address+1]] = system_ID
            address += 2
...
print(computer(Intcode, 1)['log'])

# print(computer("3,3,1107,-1,8,3,4,3,99", 10))
print(computer(Intcode, 5)['log'])
